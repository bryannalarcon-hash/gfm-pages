...
# Per-pane routing so concurrent sessions (planner / coder) don't collide in one log:
#   1. PROMPT_LOG_FILE env             → explicit per-pane override (durable across restarts).
#   2. .claude/log-route.json          → {session_id: relative_md}; session_id is the
#                                         transcript's <uuid>.jsonl basename. Seeded for
